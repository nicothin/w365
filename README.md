...
Особое внимание: на одних страницах используется (включается) [fullPage](https://github.com/alvarotrigo/fullPage.js), на других нет. Там где используется плагин, немного другая структура DOM получается — плагин ее меняет.
...
`npm run deploy`      | Отправка содержимого папки сборки на GH-pages (для корректного сообщения адреса просмотра введите его в `package.json`)
`NODE_ENV=production npm run build` | Сборка проекта с минификацией и без карт кода.
